...
final class SyncTables {
  new(this._db)
    : _upserts = {
        'households': (json) => _db
            .into(_db.households)
            .insertOnConflictUpdate(HouseholdRow.fromJson(json)),
        'accounts': (json) => _db
            .into(_db.accounts)
            .insertOnConflictUpdate(AccountRow.fromJson(json)),
        'categories': (json) => _db
            .into(_db.categories)
            .insertOnConflictUpdate(CategoryRow.fromJson(json)),
        'recurring_rules': (json) => _db
            .into(_db.recurringRules)
            .insertOnConflictUpdate(RecurringRuleRow.fromJson(json)),
        'category_limits': (json) => _db
            .into(_db.categoryLimits)
            .insertOnConflictUpdate(CategoryLimitRow.fromJson(json)),
        'quick_actions': (json) => _db
            .into(_db.quickActions)
            .insertOnConflictUpdate(QuickActionRow.fromJson(json)),
        'tags': (json) =>
            _db.into(_db.tags).insertOnConflictUpdate(TagRow.fromJson(json)),
        'debts': (json) =>
            _db.into(_db.debts).insertOnConflictUpdate(DebtRow.fromJson(json)),
        'goals': (json) =>
            _db.into(_db.goals).insertOnConflictUpdate(GoalRow.fromJson(json)),
        'months': (json) => _db
            .into(_db.months)
            .insertOnConflictUpdate(MonthRow.fromJson(json)),
        'planned_items': (json) => _db
            .into(_db.plannedItems)
            .insertOnConflictUpdate(PlannedItemRow.fromJson(json)),
        'transactions': (json) => _db
            .into(_db.transactions)
            .insertOnConflictUpdate(TransactionRow.fromJson(json)),
        'transaction_tags': (json) => _db
            .into(_db.transactionTags)
            .insertOnConflictUpdate(TransactionTagRow.fromJson(json)),
        'attachments': (json) => _db
            .into(_db.attachments)
            .insertOnConflictUpdate(AttachmentRow.fromJson(json)),
      };
  ...
  /// Server qatorini lokal jadvalga yozadi (bor bo'lsa — ustiga).
