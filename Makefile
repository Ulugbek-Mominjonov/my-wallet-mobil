...
.DEFAULT_GOAL := help
.PHONY: help check lint test fmt gen gen-check coverage integration contracts-sync contracts-check run-dev
...
integration: ## Sinxron integratsiya testlari — lokal Supabase bilan (admin repo, contracts.lock)
	tool/integration.sh
